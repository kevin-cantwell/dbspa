...
var positiveQueries = []string{
	...
	"SELECT * FROM stdin",
	"SELECT * FROM stdin e",
	"SELECT * FROM 'kafka://b/t' CHANGELOG DEBEZIUM",
	...
	"SELECT * FROM 'kafka://b/t' FORMAT AVRO CHANGELOG DEBEZIUM",
	"SELECT * FROM 'kafka://broker/topic?offset=earliest&registry=http://reg:8081'",
	// FROM without FROM (standalone select)
	"SELECT name",
	// FROM stdin with changelog
	"SELECT * FROM stdin CHANGELOG DEBEZIUM",
	// JOIN
	...
	// SEED FROM
	"SELECT region, COUNT(*) FROM 'kafka://b/t' CHANGELOG DEBEZIUM SEED FROM '/path/to/snapshot.parquet' GROUP BY region",
	...
	"SELECT _after->>'region', COUNT(*) FROM 'kafka://b/t' CHANGELOG DEBEZIUM GROUP BY _after->>'region'",
	// HAVING
	...
	"SELECT _after->>'region', COUNT(*) AS cnt FROM 'kafka://b/t' CHANGELOG DEBEZIUM GROUP BY _after->>'region' HAVING COUNT(*) > 10",
	...
	"SELECT * FROM stdin e CHANGELOG DEBEZIUM",
	...
	// Canonical FORMAT + CHANGELOG syntax
	"SELECT * FROM 'kafka://b/t' FORMAT AVRO CHANGELOG DEBEZIUM",
	"SELECT * FROM 'kafka://b/t' FORMAT JSON CHANGELOG DEBEZIUM",
	"SELECT * FROM stdin CHANGELOG DBSPA",
	"SELECT * FROM stdin FORMAT JSON CHANGELOG DBSPA",
	"SELECT * FROM 'kafka://b/t' FORMAT AVRO(registry='http://reg:8081') CHANGELOG DEBEZIUM",
	"SELECT a + b * c",
	"SELECT a OR b AND c",
	"SELECT NOT a AND b",
